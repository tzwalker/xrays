...
no_rough_iio = iio_vs_depth(ele, no_rough, dt) 
no_rough_iio = no_rough_iio.reshape(-1,1)   # format array      

plt.plot(x_for_plotting, beam_attn)

## save x and y for plotting in Origin
arr_for_plotting0 = np.concatenate((x_for_plotting, beam_attn), axis=1)
arr_for_plotting1 = np.concatenate((x_for_plotting, no_rough_iio), axis=1)
...
# no_rough_in_um = no_rough / 1000        # convert x axis to um
# 
...
# 
# np.savetxt(r'C:\Users\Trumann\Dropbox (ASU)\1_NBL3 data\iio_sim_' + str(detect_theta) +'deg_beamIN.csv', ele_rough_iios_up, delimiter=',')
# #np.savetxt(r'C:\Users\Trumann\Dropbox (ASU)\1_NBL3 data\iio_sim_' + str(detect_theta) +'deg_roughDOWN.csv', ele_rough_iios_down, delimiter=',')
