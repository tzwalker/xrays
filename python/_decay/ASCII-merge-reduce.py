...
plan view part 2
    the triple digit scans were timeseries and linescans 
    from plan-view part 2
    different h5 ascii column headers, e.g. ' ds_ic' vs. 'ds_ic'
    insert index: i=5
    repeat rows for timeseries and linescan exports
    take every 3rd row
    scans list(range(204,215)) + [218] + list(range(220,230))
...
for SCANNUM in TS1181Ascans:
    ...
    i = 5
    df_merged = pd.concat([df_h51.iloc[:, :i], df_mda1, df_h51.iloc[:, i:]], axis=1)
    
    # take every 3rd row; only use for linescans and timeseries!
    df_merged1 = df_merged[::3]
    
    OUT_PATH = r'C:\Users\triton\decay_data\csv_combined'
    MERGED_fname = r'\combined_ASCII_26idbSOFT_0{s}.h5_mda.csv'.format(s=SCANNUM)
