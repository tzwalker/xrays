...
#print(test_add.add_num(3,7))

# where i left off: want to check if element is in a given layer of interest
    # then call functions required for calculating iio of that element in that layer
        # these function will reside in play_absorb; import play_absorb

# cuts ChOI strings to make compatible with xraylib;
# for convenience as original ChOI string is used to reference within H5 struct
...
def eStringToAtomicNum(e):
    ...
    return s
# 3
def is_ele_in_layer(stack_list, e_num):
    #check_if element is in layer of stack_list
    
    # if layer contains ele_of_interest, gett iio for element in that layer
...
### old fxns to be used at some point hopefully ###
