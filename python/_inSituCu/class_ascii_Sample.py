...
class Sample():
    ...
    def import_maps_no_XBIC_conversion(self, path_ascii, CH):
        # store scan data
        for scan in self.scans:
            scan_str = str(scan)
            # define ascii of scan
    ...
    def stage_paper_XRF(self, path_ascii, CH):
        '''
        this def is needed to import ASCII from BLai fits
        to import raw ASCII:
        x pixel no and y pixel no need to have a space in their string
