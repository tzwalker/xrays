...
"""
This script does the same thing that PackageDeal_XBIC_Fluo_Meta_function.py does, except that it does not print to the terminal, and that it is used only on one scan.
The call of the program goes by: python PackageDeal_XBIC_Fluo_Meta.py SampleName ScanNumber
The python Version has to be python 2.7
SampleName is a string so it has to be written in ""
ScanNumber is an int so just write the number of the Scan you want to plot

"""


""" -----------------------Hardcoded, where the data is ----------------------"""
...
""" -----------------------Hardcoded, where the data is going ----------------------"""
...
""" -----------------------Hardcoded, where the fluo data is ----------------------"""


#Places where the files are located, and where they shall be saved for Fluorescence:
...
#Necessary Metadata for identifying the rest, first in the tuple is absolute scannumber, second is amplification value
...
if sample_dir == 'CdTe_X_TS58A':
	key= 'Run_'+str(Run)
	values=dict_CdTe_X_TS58A.get(key,(0))
	absScan=values[0]
	Amp=values[1]
elif sample_dir == 'CdTe_X_NBL33':
	key= 'Run_'+str(Run)
...
"""
def Hz2nA(data,Amp=Amp,Scale=Scale,C_Range=C_Range,S_Range=S_Range,dnorm=dnorm):
    ConFactor=C_Range*Amp*2*math.sqrt(2)/(S_Range*Scale)*dnorm
    data.data=data.data*ConFactor
    return np.array(data.data)
	
def Hz2nADirect(data,Amp=Amp,Scale=Scale,C_Range=C_Range,S_Range=S_Range,dnorm=dnorm):
    ConFactor=C_Range*Amp/(S_Range*Scale*math.sqrt(2))*dnorm
    data.data=data.data*ConFactor
    return np.array(data.data)
...
Xbic_con=Hz2nA(Xbic_raw,Scale=xbicS)
